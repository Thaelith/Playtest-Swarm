...
> Hackathon Track: AI/ML Developer Tools · Built in 24 hours
...
src/
  ...
  lib/
    ...
    cerebras.ts         — Cerebras API adapter
...
8. (0:50) Click "Social Summary" export — copy the demo-ready tweet
...
All agent logic is in `src/lib/agents.mock.ts`. When Cerebras is configured, `src/lib/cerebras.ts` sends the full multimodal prompt — image + economy JSON — to the API and parses the structured JSON response.
